...
import { Coins, LogOut, Loader2, User, Camera, ChevronRight, Shield, Download, Share, Share2, Phone } from "lucide-react";
...
export default function Profile() {
  ...
  const shareApp = async () => {
    const shareData = {
      title: profile?.shopName || "Inaamify",
      text: `Join ${profile?.shopName || "Inaamify"} to earn reward on every purchase!`,
      ...
      <div className="px-5 -mt-16 relative z-20">
        ...
        <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 mb-4 overflow-hidden">
          <button
            ...
            className="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-50 transition active:scale-[0.98]"
          >
            <div className="flex items-center gap-3">
        ...
        <div className="flex gap-3 mb-4">
          ...
          {(supportPhone || adminData?.mobile) && (
            <button
              onClick={() => {
                const phone = adminData?.mobile || supportPhone;
                const text = encodeURIComponent(`Hi, I need some help with the Redeem App at ${profile?.shopName || "your shop"}. Can you please assist me?`);
